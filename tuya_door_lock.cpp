...
char* TuyaDoorLock::getCurrentDynamicPass(uint16_t validTime, uint8_t passLen){
  static char generated_password_str[9]={0};
  #ifdef USE_TIME
      if (this->time_id_ != nullptr && this->totp_key_length_ > 0 && this->totp_key_ != nullptr && passLen>=6 && passLen<=8) {
        // Generate totp password
        ESPTime now = this->time_id_->now();
        if (now.is_valid()) {
          auto timestamp = (time_t)floor(now.timestamp / validTime);
          const uint32_t generated_password = otp::totp_hash_token(this->totp_key_, totp_key_length_, timestamp, passLen);
          char mask[]="%08u\0";
          mask[2]='0'+passLen;
          snprintf(generated_password_str, sizeof(generated_password_str), mask, generated_password);
          ESP_LOGD(TAG, "Generated TOTP len=%d password: %s", passLen,generated_password_str);
        } else {
          ESP_LOGE(TAG, "Current time is invalid or/and error TOTP key, cannot generate TOTP password.");
        }
      } else
  #endif
      {
        ESP_LOGW(TAG, "Get dynamic password is not handled because time is not configured");
      }
  return &generated_password_str[0];
}  

void TuyaDoorLock::sanityTempPass(){
   for(uint8_t i=0; i<this->tempPass.size(); i++){
      this->tempPass[i].number=i;   
   }
}

uint8_t TuyaDoorLock::addTempPass(char* pass, bool oneUse,
                                  uint8_t sYear, uint8_t sMonth, uint8_t sDay, uint8_t sHour, uint8_t sMin, uint8_t sSec,
                                  uint8_t eYear, uint8_t eMonth, uint8_t eDay, uint8_t eHour, uint8_t eMin, uint8_t eSec ){
   TuyaDoorLockTempPass tp;
   tp.number=this->tempPass.size()+1;
   if(tp.number>10) {
       ESP_LOGE(TAG,"No more than 10 passwords are allowed");
       return 0; 
   }
   tp.oneUse=oneUse;
   tp.invalid=false;
   tp.startTime.year=sYear;
   tp.startTime.month=sMonth;
   tp.startTime.day=sDay;
   tp.startTime.hours=sHour;
   tp.startTime.minutes=sMin;
   tp.startTime.seconds=sSec;
   tp.endTime.year=eYear;
   tp.endTime.month=eMonth;
   tp.endTime.day=eDay;
   tp.endTime.hours=eHour;
   tp.endTime.minutes=eMin;
   tp.endTime.seconds=eSec;
   for(uint8_t i=0; i<this->tempPass.size(); i++){
      if(memcmp(pass,this->tempPass[i].pass,7)==0){
         ESP_LOGE(TAG,"This password already exists (№ %d)",i);
         return 0;         
      }
   }
   memcpy(tp.pass,pass,7);
   this->tempPass.push_back(tp);
   uint8_t* b=(uint8_t*)&(this->tempPass[this->tempPass.size()-1]);
   this->sanityTempPass();
   return tp.number;
}

void TuyaDoorLock::delTempPass(uint8_t n){
   if(n<this->tempPass.size()){
      this->tempPass.erase(this->tempPass.begin()+n);
      this->sanityTempPass();      
   }
}
...
void TuyaDoorLock::handle_command_(uint8_t command, uint8_t version, const uint8_t *buffer, size_t len) {
  ...
  switch (command_type) {
    ...
    case TuyaDoorLockCommandType::REQUEST_TEMP_PASSWD_CLOUD_MULTIPLE:
       ...
       {
          uint8_t siz=this->tempPass.size();
          if(siz==0){ // no templ passwords
             ESP_LOGV(TAG, "No temp passwords");
             this->send_command_(
                 TuyaDoorLockCommand{
                     .cmd = TuyaDoorLockCommandType::REQUEST_TEMP_PASSWD_CLOUD_MULTIPLE,
                     .payload = std::vector<uint8_t>{0x01,0x00}});
          } else {
             std::vector<uint8_t> dataBuff;
             dataBuff.reserve(sizeof(TuyaDoorLockTempPass)*(siz)+3+1);
             dataBuff.push_back(1); // ok
             dataBuff.push_back(siz); // count passwords
             dataBuff.push_back(7); //password length
             uint8_t* inBuff=(uint8_t*)&(this->tempPass[0]);
             for(uint8_t i=0; i<siz*sizeof(TuyaDoorLockTempPass); i++){
                dataBuff.push_back(inBuff[i]);
             }
             ESP_LOGV(TAG,"Send %d temp password(s)", siz);
             this->send_command_(
                TuyaDoorLockCommand{
                   .cmd = TuyaDoorLockCommandType::REQUEST_TEMP_PASSWD_CLOUD_MULTIPLE,
                   .payload = dataBuff});
...
#ifdef USE_TIME
       if (memcmp(getCurrentDynamicPass(300,8), (char *)(buffer + 6), 8) == 0) {
          ESP_LOGV(TAG, "Password matched");
          this->send_command_(
              TuyaDoorLockCommand{
                  .cmd = TuyaDoorLockCommandType::VERIFY_DYNAMIC_PASSWORD,
                  .payload = std::vector<uint8_t>{0x00}});
       } else {
          ESP_LOGV(TAG, "Password not matched and/or totp key was incorrect");
          this->send_command_(
              TuyaDoorLockCommand{
                  .cmd = TuyaDoorLockCommandType::VERIFY_DYNAMIC_PASSWORD,
...
#else
        ESP_LOGW(TAG, "VERIFY_DYNAMIC_PASSWORD is not handled because time is not configured");
#endif
       break;
...
void TuyaDoorLock::send_raw_command_(TuyaDoorLockCommand command) {
  ...
  switch (command.cmd) {
    ...
    case TuyaDoorLockCommandType::REQUEST_TEMP_PASSWD_CLOUD_MULTIPLE:
      break;
    default:
      ESP_LOGE(TAG, "The command asked to be sent was not yet handled, cmd: %02X", command.cmd);
      break;
  }
...
void TuyaDoorLock::parse_totp_key() {
  ...
  if (actual_decoded_length > 0) {
    ...
    ESP_LOGV(TAG, "Decode totp_key_b32");
  } else {
