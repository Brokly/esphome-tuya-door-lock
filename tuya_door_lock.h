...
struct TuyaDoorLockTempPass{
   uint8_t number;
   bool oneUse;
   bool invalid;  
   TuyaDoorLockDate startTime;
   TuyaDoorLockDate endTime;
   char pass[7];
};
...
class TuyaDoorLock : public Component, public uart::UARTDevice {
 public:
  ...
  uint8_t addTempPass(char* pass, bool oneUse,
  ...
  void delTempPass(uint8_t n);
  void sanityTempPass();
  char* getCurrentDynamicPass(uint16_t validTime, uint8_t passLen);
  TuyaDoorLockInitState get_init_state();
  // I add theses here to use from lambda
  std::vector<TuyaDoorLockTempPass> tempPass; // массив временных паролей
